    ...
    useEffect(() => {
        const filtered = retreats.filter((retreat) => {
            ...
            return (
                ...
                (filters.price === '' || (retreat.price >= (minPrice || 0) && retreat.price <= (maxPrice || Infinity))) &&
                (filters.duration === '' || retreat.duration <= filters.duration) &&
                (filters.search === '' || retreat.title.toLowerCase().includes(filters.search.toLowerCase()))
            );
        });
                ...
                {currentRetreats.length > 0 ? (
                    currentRetreats.map((retreat, index) => (
                        <div className="retreat" key={index}>
                            ...
                            <h3>{retreat.title}</h3>
                            <p style={{ fontWeight: '600' }}> <span></span>{retreat.description}</p>
                            <p> <span style={{ fontWeight: '600' }}>Date:</span>{new Date(retreat.date).toDateString()}</p>
                            <p><span style={{ fontWeight: '600' }}>Location: </span> {retreat.location}</p>
                            <p><span style={{ fontWeight: '600' }}>Price: </span>${retreat.price}</p>
                            <p><span style={{ fontWeight: '600' }}>Duration: </span> {retreat.duration} days</p>
                            <p><span style={{ fontWeight: '600' }}>Type: </span> {retreat.type}</p>
